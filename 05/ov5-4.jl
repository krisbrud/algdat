...
#     for c in dna # For char in dna string
...
#     return current.count # Assume root node has count 0
# end
...
function brokendnasearch(root, dna)
    ...
    for (i, c) in enumerate(dna) # (index, char)
        if c == '?'
            # Wildcard. The number of matches equals the sum of matches in all the node's children
            if length(current.children) > 0
                # Not allowed to make empty iterator
                return sum(brokendnasearch(child, dna[(i + 1):end]) for child in values(current.children))
            else
                return 0
            end
        else
